...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile, char *filename)
{
	FILE *all_result = fopen("..\\all_result.txt", "a+");
	...
	set_batch_network(&net, 1);
	//fprintf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
	//fprintf(stderr, "FrameNo.\t     GT\t\t TruePositive\t FalsePositive\n");

	//fprintf(fp, "FrameNo.\t     FileName.\t  GT\t\t TruePositive\t FalsePositive\n");
	srand(time(0));
...
#ifdef DEBUG
	char *name_list = "G:\\darknet_오리진_TEST\\build\\darknet\\x64\\data\\3rd_PHC_nighttime.list";
	char **names = get_labels(name_list);
#else
	char *name_list = option_find_str(options, "names", "data/PHC.list");
	char **names = get_labels(name_list);
#endif
	//
	//data/3rd_PHC_nighttime.data	
	char* img_path_temp[20] = { NULL, };
	int q = 0;
	char* context = NULL;
	char* ptr = NULL;
	char data_temp_cfg[] = { NULL, };
	sprintf(data_temp_cfg, "%s", datacfg);
	//system("pause");
	ptr = strtok(data_temp_cfg, "_");
	while (ptr != NULL)
	{
		img_path_temp[q] = ptr;
		q++;
	...
	char* valid_path_temp[2] = { NULL, };
	q = 0;
	ptr = strtok(img_path_temp[2], ".");
	while (ptr != NULL)
	{
		valid_path_temp[q] = ptr;
		q++;
		ptr = strtok(NULL, ".");
	}
	char valid_images_path[256];
	sprintf(valid_images_path, "data/%s_valid.txt", valid_path_temp[0]);

	//char *valid_images = option_find_str(options, "valid", "data/nighttime_valid.txt");
	char *valid_images = option_find_str(options, "valid", valid_images_path);
	...
	int total_frame_number = cvGetCaptureProperty(capture, CV_CAP_PROP_FRAME_COUNT);
	...
	char* directory;
	char temp_paths[256] = "";
	sprintf(temp_paths, "%s", paths[0]);
	char *save_image_name = strtok(paths[0], "/");
	int idx_cnt = 0;
	while (save_image_name != NULL)
	{
		file_path[idx_cnt] = save_image_name;
		idx_cnt++;
		save_image_name = strtok(NULL, "/");
	}
	paths[0] = temp_paths;
	int nResult;
	char* full_path;
	char posix[256];
	char *add_path = "_detection_Result\\";
	sprintf(posix, ".\\%s%s", file_path[1], add_path);
	system("pause");
	char result_txt_file[256];
	char result_txt[19] = "Validation_result";
	sprintf(result_txt_file, "%s_%s.txt", result_txt, file_path[1]);
	FILE* fp = fopen(result_txt_file, "w");
	fprintf(stderr, "%7s\t\t%18s\t%2s\t%12s\t%13s\n", "FrameNo", "FrameName", "GT", "TruepPositive", "FalsePositive");
	fprintf(fp, "%7s\t\t%18s\t%2s\t%12s\t%13s\n", "FrameNo", "FrameName", "GT", "TruepPositive", "FalsePositive");

	//결과이미지 저장할 디렉토리가 존재하지않으면 새로 생성
	if ((nResult = _access(posix, 0)) == -1)
		_mkdir(posix);
	//IplImage* test_img;
	//Validation 시작
	for (i = 0; i < m - 1; ++i) {
		int cnt__ = 0;
		save_image_name = strtok(paths[i], "/");
		...
		char path[1024] = { NULL, };
		char temp_path[1024] = { NULL, };
		sprintf(path, "%s/%s/%s", file_path[0], file_path[1], file_path[2]);
		sprintf(temp_path, "%s", path);
		save_image_name = file_path[2];

		int current_frame = cvGetCaptureProperty(capture, CV_CAP_PROP_POS_FRAMES);

		image orig = get_image_from_stream(capture);
		...
		//
		//ST BOX draw
		int display_classlabel;
		if ((strcmp(file_path[1], "Daytime_GT") == 0) || (strcmp(file_path[1], "Caltech_GT")) == 0)
			display_classlabel = 0;
		else
			display_classlabel = 1;
		int allDetected = 0;
		int class_label = 0;
		...
		//allDetected = draw_detections_PHC(orig, l.w*l.h*l. n, 0.4, dets, temp_boxes, probs, l.classes, display_classlabel);
		allDetected = draw_detection_cv_v3_PHC(orig, dets, temp_boxes, nboxes, thresh, l.classes, display_classlabel);
		box *det_boxes = calloc(20, sizeof(box));
		//for (int u = 0; u < allDetected; u++)
		for (int u = 0; u < allDetected; u++)
			det_boxes[u] = temp_boxes[u];
/*		for(int o = 0; o < nboxes; o++)
			for (int u = 0; u < 20; u++)
			{
				if (temp_boxes[u].x != 0)
				{
					det_boxes[o].x = temp_boxes[u].x;
					det_boxes[o].y = temp_boxes[u].y;
					det_boxes[o].w = temp_boxes[u].w;
					det_boxes[o].h = temp_boxes[u].h;
					det_count++;
				}
			}
		allDetected = det_count*/;
	/*	for (int u = 0; u < 20; u++)
		{
			if (temp_boxes[u].x != 0)
			{
				det_boxes[u].x = temp_boxes[u].x;
				det_boxes[u].y = temp_boxes[u].y;
				det_boxes[u].w = temp_boxes[u].w;
				det_boxes[u].h = temp_boxes[u].h;
			}
		}*/
	
		char labelpath[4096] = { NULL, };
		...
		find_replace(temp_path, "jpg", "txt", labelpath);
		int num_labels = 0;
		int current_TP = 0;
		int current_FP;

		box_label *truth = read_boxes(labelpath, &num_labels); //GT BOX
															   //printf("labelPath :%s\n", labelpath);
		int personLabel_Count = 0;
		for (int k = 0; k < num_labels; k++)
		{
			if (display_classlabel == 0)
				personLabel_Count++;
			else
				personLabel_Count++;
		}

		box *GTBOX = calloc(personLabel_Count, sizeof(box));
		int cnt = 0;
		for (int y = 0; y < num_labels; y++)
		{
			if (display_classlabel == 0)
			{
				GTBOX[cnt].x = truth[y].x;
				GTBOX[cnt].y = truth[y].y;
				GTBOX[cnt].w = truth[y].w;
				GTBOX[cnt].h = truth[y].h;
				cnt++;
			}
			else
			{
		...
		draw_detections_PHC_GT(orig, personLabel_Count, GTBOX);
		//save_image(orig, "Detection Result");
		char split_save_image_name[256] = { NULL, };
		find_replace(save_image_name, ".jpg", "", split_save_image_name);
#ifdef SAVE_IMAGE
		save_image_jpg_PHC(orig, i + 1, posix, split_save_image_name);
#endif
		free(GTBOX);
			...
			if (display_classlabel == 0)
			{
				if (truth[j].id != 9)
				{
					...
					if (best_iou > iou_thresh) {
						++current_TP;
						if (current_TP > allDetected)
							current_TP = allDetected;
						++correct;
					}*/
					box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h };
					float best_iou = 0;
					//for (k = 0; k < l.w*l.h*l.n; ++k) {
					for (k = 0; k < allDetected; ++k) {
						//float iou = box_iou(boxes[k], t);
						float iou = box_iou(det_boxes[k], t);

						//if (iou > iou_thresh)
						//if (probs[k][1] > thresh && iou > best_iou) {
						if (iou > best_iou) {
							best_iou = iou;
						}
					}
		...
		free(det_boxes);
		//correct		: 전체 TP 개수
		//num_labels	: 프레임당 GT개수
		//total			: 전체 GT 개수
		//i				: 테스트 이미지 개수
		//truth_count	: 전체 GT 개수
		//fprintf(stderr, "i : %5d TP : %5d  total : %5d TP : %5d \tRPs/Img: %.2f\tIOU: %.2f%%\tRecall:%.2f%%\n", i, correct, total,true_positive, 
		//	(float)proposals/(i+1), avg_iou*100/total, 100.*correct/total);
		current_FP = allDetected - current_TP;
		totalFP += current_FP;
		//fprintf(stderr,"image No : %5d totalTP : %5d currentTP : %5d currentFP : %5d currentGT : %5d\n",i,correct, current_TP,current_FP, num_labels );
		//fprintf(stderr, "%5d\t%s\t %5d\t\t %5d\t\t %5d \n", i + 1, save_image_name,personLabel_Count, current_TP, current_FP);

		fprintf(stderr, "%7d\t\t%18s\t%2d\t%12d\t%13d\n", i + 1, save_image_name, personLabel_Count, current_TP,
			current_FP);
		fprintf(fp, "%7d\t\t%18s\t%2d\t%12d\t%13d\n", i + 1, save_image_name, personLabel_Count, current_TP,
			current_FP);
		free(id);
	...
	printf("===================================================================================\n");
	fprintf(fp, "===================================================================================\n");
	printf("%s\t\t%18s\t%2s\t%12d\t%13d\n", "Total", "", "", correct, totalFP);
	fprintf(fp, "%s\t\t%18s\t%2s\t%12d\t%13d\n", "Total", "", "", correct, totalFP);
	printf("Precision\t\t%.2f \n", (correct *100.0) / (correct + totalFP));
	fprintf(fp, "Precision\t\t%.2f \n", (correct *100.0) / (correct + totalFP));
	//printf("\n truth_count = %d \n", truth_count);
	cvReleaseCapture(&capture);
	//cvReleaseImage(&test_img);
	fprintf(all_result, "%s\t%lf\n", filename, (correct*100.0) / (correct + totalFP));
	fclose(all_result);
	fclose(fp);
	free(boxes);
	for (int q = 0; q < l.w*l.h*l.n; q++)
		free(probs[q]);
	free(probs);
	//system("pause");
}
...
void run_detector(int argc, char **argv)
{
    ...
    else if (0 == strcmp(argv[2], "recall")) validate_detector_recall(datacfg, cfg, weights, filename);
